# Project: arcade cashier

## 1. Architecture & Folder Structure
**Style:** Feature-First, Riverpod Architecture (CodeWithAndrea style).
**Root:** `lib/src/`
- **`features/`**: Contains feature-based modules (e.g., `authentication`, `dashboard`, `rooms`, `sessions`).
  - Inside each feature:
...
- **`common_widgets/`**: Reusable UI components (Buttons, Cards).
...
- **Responsiveness:** `flutter_adaptive_scaffold` or `layout_builder` for adaptive layouts (Mobile, Tablet, Desktop).
...
- **Language:** Support RTL (Right-to-Left) for Arabic.
...
## 4. Resource Management (Zero Hard-Coding)
  - Strings: NEVER use hard-coded strings in Text widgets or logic. All strings must be defined in an abstract class (AppStrings).

  - Routes: Route names and paths must be defined as constants in an abstract class (AppRoutes). Never type a raw route string in GoRouter configuration or navigation calls.
